...
import { Brain, Briefcase, Code, ArrowRight, ArrowLeft, Check, AlertTriangle } from 'lucide-react';
...
const InstallationWizard = () => {
  ...
  const handleNext = () => {
    ...
    if (!stepStates[currentStepId] && currentStep < steps.length - 1) {
      setShowWarning(true);
    } else {
      setCurrentStep(prev => prev + 1);
    }
      ...
      case 'system':
        return (
          <SystemCheck
            ...
            onComplete={(success) => handleStepComplete('system', success)}
          />
        );
      ...
      <div className="min-h-[400px] p-6 bg-dark-card rounded-lg">
        ...
        {showWarning && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4">
            <div className="bg-dark-card rounded-lg p-6 max-w-md w-full space-y-4">
              <div className="flex items-start space-x-3">
                <AlertTriangle className="w-6 h-6 text-concept-yellow flex-shrink-0" />
                <div>
                  <h3 className={`${styles.heading} text-lg`}>Incomplete Step</h3>
                  <p className={`${styles.text} mt-2`}>
                    This step has not been fully completed. Some features may not be properly configured if you proceed.
                  </p>
                </div>
              </div>
