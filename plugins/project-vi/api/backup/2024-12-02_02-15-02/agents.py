...
def structure_node(state: VaultState) -> Dict:
    ...
    if not state.messages:
        state.messages = [
            HumanMessage(content=f"Please analyze the vault structure and provide insights about its organization. Use the tools with this state data: {state_dict}")
        ]
    
    result = agent.invoke({"messages": state.messages, "state": state_dict})
